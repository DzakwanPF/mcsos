...
if [ ! -f "$ISO" ]; then
  echo "ERROR: $ISO tidak ditemukan. Jalankan make image." >&2; exit 1
fi
if [ -z "$OVMF_CODE" ]; then
  echo "ERROR: OVMF_CODE tidak ditemukan. Pasang paket ovmf." >&2; exit 1
fi
...
if [ "$status" != "0" ] && [ "$status" != "124" ]; then
  echo "ERROR: QEMU keluar dengan status $status" >&2; exit "$status"
fi

if [ ! -s "$LOG" ]; then
  echo "ERROR: serial log kosong: $LOG" >&2; exit 1
fi
