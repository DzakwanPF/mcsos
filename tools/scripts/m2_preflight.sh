...
case "$ROOT" in
  /mnt/c/*|/mnt/d/*|/mnt/e/*)
    fail "repository berada di filesystem Windows. Pindahkan ke filesystem Linux WSL."
    ;;
...
if [ -f build/meta/toolchain-versions.txt ]; then
  ...
  if make -n meta >/dev/null 2>&1; then make meta
  else fail "target make meta tidak tersedia dan metadata M1 belum ada"
  fi
fi
