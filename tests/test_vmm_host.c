...
int main(void) {
    ...
    assert(m.flags & VMM_PTE_PRESENT);
    assert(m.flags & VMM_PTE_WRITABLE);
    assert(m.flags & VMM_PTE_NO_EXECUTE);

    /* duplicate map harus gagal */
    assert(vmm_map_page(&space, 0xFFFF800000200000ULL, 0x0000000000400000ULL, 0) == VMM_ERR_EXISTS);

    /* unaligned paddr harus gagal */
    assert(vmm_map_page(&space, 0xFFFF800000201000ULL, 0x0000000000400001ULL, 0) == VMM_ERR_INVAL);

    /* noncanonical vaddr harus gagal */
