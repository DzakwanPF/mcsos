...
static void zero_context(mcsos_context_t *context) {
    ...
    context->r12 = 0; context->r13 = 0; context->r14 = 0;
    context->r15 = 0; context->rip = 0;
}
...
int mcsos_scheduler_init(mcsos_scheduler_t *sched, mcsos_thread_t *boot_thread) {
    ...
    boot_thread->id = 0; boot_thread->name = "boot";
    ...
    boot_thread->switches = 0; boot_thread->ticks = 0; boot_thread->exit_code = 0;
    zero_context(&boot_thread->context);
    sched->current = boot_thread; sched->idle = boot_thread;
    sched->ready_head = (mcsos_thread_t *)0;
    sched->ready_tail = (mcsos_thread_t *)0;
    sched->next_id = 1; sched->runnable_count = 0;
    sched->context_switches = 0; sched->ticks = 0; sched->initialized = 1;
...
int mcsos_thread_prepare(mcsos_thread_t *thread, const char *name,
    ...
    *((uint64_t *)top) = UINT64_C(0);
    thread->magic = MCSOS_THREAD_MAGIC; thread->id = id; thread->name = name;
...
int mcsos_sched_enqueue(mcsos_scheduler_t *sched, mcsos_thread_t *thread) {
    ...
    if (sched->ready_tail == (mcsos_thread_t *)0) {
        sched->ready_head = thread; sched->ready_tail = thread;
    } else {
        sched->ready_tail->next = thread; sched->ready_tail = thread;
    }
...
mcsos_thread_t *mcsos_sched_pick_next(mcsos_scheduler_t *sched) {
    ...
    if (thread == (mcsos_thread_t *)0) return sched->idle;
    sched->ready_head = thread->next;
    if (sched->ready_head == (mcsos_thread_t *)0) sched->ready_tail = (mcsos_thread_t *)0;
    thread->next = (mcsos_thread_t *)0;
...
int mcsos_sched_yield(mcsos_scheduler_t *sched) {
    ...
    if (!valid_thread_object(next_thread)) return MCSOS_SCHED_ECORRUPT;
    if (next_thread == old_thread) { old_thread->state = MCSOS_THREAD_RUNNING; return MCSOS_SCHED_OK; }
    ...
    old_thread->switches++; next_thread->switches++;
...
int mcsos_sched_tick(mcsos_scheduler_t *sched) {
    if (sched == (mcsos_scheduler_t *)0 || sched->initialized == 0 || !valid_thread_object(sched->current))
        return MCSOS_SCHED_EINVAL;
    sched->ticks++; sched->current->ticks++;
    return MCSOS_SCHED_OK;
}
...
size_t mcsos_sched_ready_count(const mcsos_scheduler_t *sched) {
    if (sched == (const mcsos_scheduler_t *)0 || sched->initialized == 0) return 0u;
    size_t count = 0u;
    const mcsos_thread_t *cursor = sched->ready_head;
    while (cursor != (const mcsos_thread_t *)0) { count++; cursor = cursor->next; }
    return count;
}
...
int mcsos_sched_validate(const mcsos_scheduler_t *sched) {
    ...
    while (cursor != (const mcsos_thread_t *)0) {
        if (!valid_thread_object(cursor) || cursor->state != MCSOS_THREAD_READY) return MCSOS_SCHED_ECORRUPT;
