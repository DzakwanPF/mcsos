...
void mcsos_syscall_init(const mcsos_syscall_ops_t *ops) {
    ...
    if (ops != 0) {
        if (ops->get_ticks)     g_ops.get_ticks     = ops->get_ticks;
        if (ops->yield_current) g_ops.yield_current = ops->yield_current;
        if (ops->exit_current)  g_ops.exit_current  = ops->exit_current;
        if (ops->write_serial)  g_ops.write_serial  = ops->write_serial;
    }
}
...
int mcsos_user_check_range(uintptr_t addr, size_t len) {
    ...
    if (last < addr)                return 0;  /* overflow check */
...
int mcsos_copy_from_user(void *dst, const void *src, size_t len) {
    ...
    if (!dst || !src) return MCSOS_EINVAL;
...
static int64_t sys_yield(uint64_t a0,uint64_t a1,uint64_t a2,
                         uint64_t a3,uint64_t a4,uint64_t a5){
    (void)a0;(void)a1;(void)a2;(void)a3;(void)a4;(void)a5;
    if (!g_ops.yield_current) return MCSOS_EBUSY;
    g_ops.yield_current();
    return MCSOS_OK;
}
static int64_t sys_exit_thread(uint64_t code,uint64_t a1,uint64_t a2,
    ...
    if (!g_ops.exit_current) return MCSOS_EBUSY;
